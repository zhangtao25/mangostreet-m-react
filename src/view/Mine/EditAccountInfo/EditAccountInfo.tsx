    ...
    return (
        <div>
          ...
          <List className="my-list">
            ...
            <Item arrow="horizontal" extra={this.state.info.user_id} onClick={() => {}}>小红书号</Item>
            <Item arrow="horizontal" extra={this.state.info.user_sex?'男':'女'} onClick={()=>{this.showModal()}}>性别</Item>
            <Item arrow="horizontal" extra="1995-10-24" onClick={() => {}}>生日</Item>
          </List>
          ...
          <Modal
              ...
              title="选择性别"
              footer={[{ text: '确认', onPress: () => { console.log('确认'); this.onClose(); } }]}
          >
            <div>
              <List>
                {data.map(i => (
                    <RadioItem key={i.value} checked={this.state.value2 === i.value} onChange={() => this.onChange(i.value)}>
                      {i.label}
                    </RadioItem>
                ))}
              </List>
